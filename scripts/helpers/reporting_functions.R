...
enrich_wcs <- function(){
        ...
        wcs <- wcs %>% 
                mutate(china_zip_clean = case_when(country_text == "China" ~ str_sub(as.character(SHIPPING_ZIPCODE),end = -3), TRUE ~ "")) %>% 
                left_join(cities, by = "china_zip_clean") %>% 
                mutate(normalized_city = case_when(country_text == "China" ~ City, TRUE ~ SHIPPING_CITY))
